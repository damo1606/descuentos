...
const METHODOLOGY_INTROS: Record<Tab, { what: string; how: string; output: string }> = {
  ...
  "METODOLOGÍA 2": {
    ...
    how: "Cada strike recibe un z-score de su GEX total y su PCR. La suma de ambos z-scores genera un 'Institutional Pressure Score'. Soporte = strike bajo spot con GEX positivo y PCR > 1. Resistencia = strike sobre spot con GEX negativo y PCR < 1.",
  ...
  "METODOLOGÍA 3": {
    ...
    how: "Cada vencimiento contribuye con peso exp(−DTE/45): vencimientos cercanos pesan más. Los tres ejes se normalizan con z-score. El 'Confluence Score' suma los tres z-scores — cuanto mayor la magnitud, más fuerte el nivel institucional.",
  ...
  "METODOLOGÍA 4": {
    ...
    how: "Cada celda (strike × vencimiento) muestra el OI total con intensidad de color proporcional. La capa de IV Skew superpone el diferencial call/put IV por strike.",
  ...
  "METODOLOGÍA 5": {
    ...
    how: "Pondera GEX × OI × PCR con tiempo de vencimiento. Los niveles de M2 y M3 se comparan con los de M5 — cuando los tres modelos convergen en el mismo precio, la señal es de máxima confianza.",
  ...
  "METODOLOGÍA 6": {
    ...
    how: "Cuatro señales: VIX nivel (35%), estructura de plazos VIX/VIX3M (25%), SPY GEX total (30%) y SPY PCR (10%). En PÁNICO o CRISIS las señales de GEX se suspenden automáticamente.",
    output: "Régimen detectado (COMPRESIÓN / TRANSICIÓN / EXPANSIÓN / PÁNICO AGUDO / CRISIS SISTÉMICA), multiplicador sobre el score de M5, Brief Operativo ajustado por régimen.",
  },
  "METODOLOGÍA 7": {
    what: "Veredicto final consolidado — agrega las seis metodologías en un único score (−100 a +100): M5 (35%), M6 (25%), M2 (20%), M3 (15%), M1 (5%).",
    how: "El régimen de mercado (M6) actúa como multiplicador global: en COMPRESIÓN amplifica (×1.2), en EXPANSIÓN reduce (×0.7), en PÁNICO/CRISIS suspende. Los niveles S/R se agrupan por proximidad (±0.5%) para identificar zonas de máxima confluencia.",
    output: "Score unificado y veredicto, tabla de S/R institucional con votos (0-4 metodologías), setups PRIMARY LONG y PRIMARY SHORT, matriz de timing a 4 marcos temporales.",
  },
}
...
export default function WallModal({ open, onClose }: WallModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/75" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div
        ref={panelRef}
        className="fixed inset-0 bg-bg text-text overflow-y-auto"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b-2 border-accent bg-surface">
          <div className="flex items-center gap-3">
            <span className="text-emerald-400 font-bold text-sm tracking-[0.3em] select-none">WALL</span>
            {analyzeKey > 0 && (
        ...
        {/* Controls */}
        <div className="border-b border-border px-4 sm:px-6 py-3 bg-surface sticky top-0 z-40 shadow-sm">
        ...
        {/* Content — always mounted to preserve state when switching tabs */}
