...
function ConfBar({ value, max = 100 }: { value: number; max?: number }) {
  ...
  const color = pct >= 70 ? "bg-emerald-500" : pct >= 40 ? "bg-yellow-500" : "bg-gray-600";
  return (
    <div className="flex items-center gap-1.5">
...
type TabId = "resumen" | "m1" | "m2" | "m3";
const TABS: { id: TabId; label: string; desc: string }[] = [
  { id: "resumen", label: "RESUMEN",     desc: "Score fundamental + convicción combinada" },
  { id: "m1",      label: "M1 · GEX",   desc: "Gamma Exposure · Presión institucional · Dealer Flow" },
  { id: "m2",      label: "M2 · Z-SCORE", desc: "Z-Score GEX + PCR · Soporte/Resistencia estadístico" },
  { id: "m3",      label: "M3 · CONFLUENCIA", desc: "Multi-expiración · Confianza de niveles S/R" },
];
      ...
      <div className="border-b border-border px-4 sm:px-6 py-4 bg-surface">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-baseline gap-3 mb-1">
            ...
            <span className="text-xs text-muted tracking-widest">FUNDAMENTALES × M1 · M2 · M3</span>
          </div>
          <p className="text-xs text-subtle leading-relaxed max-w-2xl">
            Filtra por score fundamental de Descuentos y valida cada acción con tres metodologías de opciones:
            GEX (M1), Z-Score institucional (M2) y confluencia multi-expiración (M3).
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-xs text-muted tracking-widest">Obteniendo M1 + M2 + M3 para {limit} tickers...</p>
            <p className="text-[10px] text-muted/60">Esto puede tomar 15–25 segundos</p>
          </div>
        )}
        ...
        {sorted.length > 0 && !loading && (
          <>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm border-collapse">
                  <thead><tr className="border-b border-border bg-surface">
                    ...
                    <Th label="BIAS SORE"   sortKey="soreBias"        current={sortKey} asc={sortAsc} onSort={handleSort} />
            ...
            <div className="mt-6 flex flex-wrap gap-x-6 gap-y-1 text-[10px] text-muted tracking-widest border-t border-border pt-4">
              <span>CONVICCIÓN = Desc.(40%) + M1(20%) + M2(15%) + M3(15%) + Confluencia GEX(10%)</span>
